--  1、创建数据库
-- 指定utf8字符创建
create database fucking character set utf8;
...
create user 'FW'@'%' identified by '1234zxcv';

--  3、授权用户
-- 将数据库wordpress下的所有（*）授权给userwordpress
grant all privileges on fucking.* to 'FW'@'%';
grant all privileges on common.* to 'FW'@'%';
grant all privileges on tuling.* to 'FW'@'%';
-- 最后刷新权限
flush privileges;
...
grant all on fucking.* to 'FW'@'%' identified by '1234zxcv';
flush privileges;
